...
Cada objeto del catalogo se dibuja aqui. `objects.tsx` tiene un componente por tipo; `Frame`
envuelve a todos con la cabecera, el icono, el subtitulo y los complementos.

| Archivo | Que es |
|---|---|
| `objects.tsx` | Un render por tipo de objeto, mas `Frame`, `Multiples` y los ayudantes comunes |
| `Chart.tsx` | Lee los colores del tema de las variables CSS y monta el lienzo |
| `Canvas.tsx` | El unico modulo que importa ECharts. Carga diferida |
| `SortableTable.tsx`, `MatrixTable.tsx` | Tabla llana y matriz jerarquica |
| `elements.tsx`, `containers.tsx` | Elementos y contenedores, que no consumen datos |
...
3. **El color sale del tema**, leido de las variables CSS en `Chart.tsx`. No se escribe un
   literal.
...
`apps/shell/src/server/locale.ts`. Los numeros y las fechas salen de `t.numero` y `t.fecha`, nunca de
...
- No importar ECharts fuera de `Canvas.tsx`: se rompe la carga diferida.
