...
| `TablaOrdenable.tsx`, `TablaDeMatriz.tsx` | Tabla llana y matriz jerarquica |
...
1. **Respaldo accesible siempre.** Un `<canvas>` no tiene nada que un lector de pantalla pueda
   recorrer. Todo grafico lleva su tabla equivalente, envuelta en `TablaDeRespaldo`, que aporta
   `tabIndex`, `role` y nombre accesible.
2. **Filtrado cruzado por raton Y por teclado** (4.4). Las celdas de categoria se dibujan con
   `CeldaDeCategoria`, que produce un boton alcanzable con el tabulador.
...
- No escribir una tabla de respaldo a mano: usa `TablaDeRespaldo` y `CeldaDeCategoria`.
