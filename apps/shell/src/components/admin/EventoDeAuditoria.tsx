...
export function EventoDeAuditoria({ evento }: { evento: ConfigChangeLog }) {
  return (
    <li className="registro__fila" data-ampliacion={evento.isScopeExpansion ? 'si' : undefined}>
      <span className="registro__cuando">
      ...
      <span className="registro__que">
        <strong>{evento.actorId}</strong> {ACCION[evento.action] ?? evento.action}{' '}
      ...
      */}
      {evento.justification ? (
        <span className="registro__motivo">{evento.justification}</span>
      ) : null}
    </li>
  );
}
