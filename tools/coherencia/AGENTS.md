# tools/coherencia — los dos lados de un contrato que nadie ata
...
- **Toda lista de archivos se cuenta antes de compararla.** Una guarda sin entrada sale verde
  siempre: el patron `apps/shell/app/**/*.css` no devolvia ningun archivo —`**` exige una carpeta
  intermedia y `globals.css` cuelga directo de `app`— y la comprobacion de los `data-*` llevaba
  desde el primer dia comparando cero selectores contra el TSX. Cada bloque abre con un
  `it` que afirma que hay algo que revisar.
