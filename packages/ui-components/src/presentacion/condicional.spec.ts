...
const reglas = (...r: FormatoCondicional['reglas']): FormatoCondicional => ({ reglas: r });
...
describe('colorCondicional', () => {
  ...
  it('un nulo NO entra en ninguna regla, ni siquiera en «menor que»', () => {
     ...
     */
    const f = reglas({ comparador: 'menor', valor: 10, color: 'error' });
  ...
  it('una regla con medida solo se aplica a ESA medida', () => {
    ...
    const f = reglas({ medida: 'DiasResolucion', comparador: 'mayor', valor: 90, color: 'error' });
  ...
  it('sin medida declarada, se aplica a todas', () => {
    const f = reglas({ comparador: 'mayor', valor: 90, color: 'error' });
  ...
  it('«entre» incluye los dos extremos y no depende del orden en que se escriban', () => {
    const f = reglas({ comparador: 'entre', valor: 90, hasta: 30, color: 'terciario' });
  ...
  it('«entre» sin el otro extremo no casa nunca', () => {
    ...
    expect(colorCondicional(reglas({ comparador: 'entre', valor: 90, color: 'error' }), 90)).toBeUndefined();
  });
