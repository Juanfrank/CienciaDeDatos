...
import { MAX_PANELS, columnsFor, maxCommon, splitMultiples } from './multiples';
...
describe('partir en paneles', () => {
  it('el primer trozo de la etiqueta es el panel y el segundo la categoria', () => {
    ...
    expect(panels.map((p) => p.titulo)).toEqual(['Penal', 'Civil']);
    expect(panels[0]?.vm.points.map((p) => p.label)).toEqual(['Q1', 'Q2']);
    expect(panels[1]?.vm.points).toHaveLength(1);
  });
  ...
  it('una categoria que lleva el separador dentro no se parte dos veces', () => {
     ...
     */
    const { panels } = splitMultiples(vm([['Penal / Ordinario / Q1', 10]]));
    expect(panels[0]?.titulo).toBe('Penal');
    expect(panels[0]?.vm.points[0]?.label).toBe('Ordinario / Q1');
  });
  ...
  it('sin segunda dimension devuelve un solo panel, no cero', () => {
    ...
    const { panels } = splitMultiples(original);
    expect(panels).toHaveLength(1);
    expect(panels[0]?.vm).toBe(original);
  });
});
...
describe('escala comun', () => {
  it('el maximo sale de TODOS los paneles, que es lo que los hace comparables', () => {
     ...
     */
    const { panels } = splitMultiples(vm([['A / x', 20], ['B / x', 2000]]));
    expect(maxCommon(panels)).toBe(2000);
  });

  it('los nulos no entran: no son cero', () => {
    const { panels } = splitMultiples(vm([['A / x', null], ['B / x', 30]]));
    expect(maxCommon(panels)).toBe(30);
  });
  ...
  it('sin ni un valor NO se inventa un maximo', () => {
    ...
    expect(maxCommon(splitMultiples(vm([['A / x', null]])).panels)).toBeUndefined();
  });
});
...
describe('la rejilla', () => {
  ...
  it('nunca pasa de cuatro columnas, aunque se pidan mas', () => {
    ...
    expect(columnsFor(25)).toBe(4);
    expect(columnsFor(6, 8)).toBe(4);
  });

  it('lo pedido manda sobre lo deducido', () => {
    expect(columnsFor(9, 2)).toBe(2);
  });
});
...
describe('el limite de paneles', () => {
  ...
  it('no se montan mas de doce: cada panel es una instancia de ECharts', () => {
     ...
     */
    const { panels } = splitMultiples(muchos(50));
  ...
  it('y lo que no cabe se CUENTA, para poder decirlo', () => {
    ...
    expect(splitMultiples(muchos(50)).omitted).toBe(50 - MAX_PANELS);
    expect(splitMultiples(muchos(3)).omitted).toBe(0);
  });
  ...
  it('se quedan los PRIMEROS del orden vigente, no los mayores', () => {
     ...
     */
    const { panels } = splitMultiples(muchos(20));
    expect(panels[0]?.titulo).toBe('G0');
    expect(panels.at(-1)?.titulo).toBe(`G${MAX_PANELS - 1}`);
  });
  ...
  it('la escala comun sale de los paneles QUE SE DIBUJAN', () => {
     ...
     */
    const { panels } = splitMultiples(muchos(50));
    expect(maxCommon(panels)).toBe(MAX_PANELS);
  });
});
