...
`tools/load/plan-de-carga.mts` define el escenario —que rutas, con que pesos, que se mide— y se puede ejecutar tal cual:

```
npx tsx tools/load/plan-de-carga.mts --url http://localhost:4310 --concurrencia 20 --segundos 20
```
