...
- **No hay GET en `/api/reset`.** Un endpoint para comprobar si un resetId existe seria un oraculo para tantear tokens sin gastar intentos.
...
- **Queda escrito que el break-glass es un procedimiento, no codigo** (`docs/operations/acceso-de-emergencia.md`), y que el hueco que lo hace necesario —el panel permite dejar la institucion sin ningun Administrador— es evitable en el codigo y esta anotado en la hoja de ruta.
