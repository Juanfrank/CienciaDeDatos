...
`moduleLoad` comprueba que el EQUIPO tenga concedido el modulo en el arbol, y un borrador no
esta en el arbol. La vista previa se salta esa comprobacion —la autorizacion para ver un borrador
es «es tuyo», y la hace `visibleModuleSlug`— pero **usa el mismo `readObjects` y el mismo
