...
- **Si algun dia entra un modelo de lenguaje**, la puerta de 4.2 no depende de confiar en el: la pone la FORMA del resultado. Un adaptador solo puede devolver una `ConsultaResuelta` sobre el vocabulario permitido, nunca una consulta libre. Lo que si habra que decidir entonces es si el texto de la pregunta puede salir del perimetro institucional, que es una decision de la institucion y no del codigo.
